/*
 * * Copyright 2018-2019 github.com/ReflxctionDev
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package io.github.reflxction.hypixelapi.statistics

import com.google.gson.annotations.SerializedName
import java.util.*

/**
 * Represents all the Blitz Hunger Games statistics of a player.
 */
@StatisticName("HungerGames")
class BlitzStatistics {

    val arachnologist: Int = 0

    val archer: Int = 0

    val armorer: Int = 0

    val astronaut: Int = 0

    val baker: Int = 0

    val blaze: Int = 0

    @SerializedName("chosen_taunt")
    val chosenTaunt: String = ""

    @SerializedName("chosen_victorydance")
    val chosenVictorydance: String = ""

    val coins: Int = 0

    val creepertamer: Int = 0

    val deaths: Int = 0

    val fisherman: Int = 0

    val horsetamer: Int = 0

    val hunter: Int = 0

    val kills: Int = 0

    val knight: Int = 0

    val meatmaster: Int = 0

    val necromancer: Int = 0

    val packages: List<Any> = Collections.emptyList()
    val pigman: Int = 0

    val reddragon: Int = 0

    val rogue: Int = 0

    val scout: Int = 0

    val slimeyslime: Int = 0

    val speleologist: Int = 0

    val tim: Int = 0

    val toxicologist: Int = 0

    val troll: Int = 0

    val wins: Int = 0

    val wolftamer: Int = 0

    @SerializedName("wins_teams")
    val winsTeams: Int = 0

    val aura: String = ""

    @SerializedName("monthly_kills_b")
    val monthlyKillsB: Int = 0

    @SerializedName("votes_Egypt")
    val votesEgypt: Int = 0

    @SerializedName("votes_Caelum v2")
    val votesCaelumV2: Int = 0

    @SerializedName("votes_Despair v1")
    val votesDespairV1: Int = 0

    @SerializedName("weekly_kills_b")
    val weeklyKillsB: Int = 0

    @SerializedName("weekly_kills_a")
    val weeklyKillsA: Int = 0

    val paladin: Int = 0

    @SerializedName("shadow knight")
    val shadowKnight: Int = 0

    @SerializedName("kit_permutations_creepertamer")
    val kitPermutationsCreepertamer: String = ""

    @SerializedName("kit_permutations_blaze")
    val kitPermutationsBlaze: String = ""

    @SerializedName("monthly_kills_a")
    val monthlyKillsA: Int = 0

    @SerializedName("votes_Mirador Basin")
    val votesMiradorBasin: Int = 0

    @SerializedName("kit_permutations_archer")
    val kitPermutationsArcher: String = ""

    val defaultkit: String = ""

    @SerializedName("kit_permutations_slimeyslime")
    val kitPermutationsSlimeyslime: String = ""

    @SerializedName("rambo_wins")
    val ramboWins: Int = 0

    @SerializedName("chosen_finisher")
    val chosenFinisher: String = ""

    @SerializedName("hype train")
    val hypeTrain: Int = 0

    @SerializedName("votes_Caelum")
    val votesCaelum: Int = 0

    val jockey: Int = 0

    @SerializedName("votes_Aelin's Tower")
    val votesAelinsTower: Int = 0

    @SerializedName("kit_permutations_pigman")
    val kitPermutationsPigman: String = ""

    @SerializedName("kit_permutations_scout")
    val kitPermutationsScout: String = ""

    @SerializedName("kit_permutations_hunter")
    val kitPermutationsHunter: String = ""

    @SerializedName("TrollInventory")
    val trollinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("CreepertamerInventory")
    val creepertamerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("ScoutInventory")
    val scoutinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("ArcherInventory")
    val archerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("HunterInventory")
    val hunterinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("combatTracker")
    val combattracker: Boolean = false

    @SerializedName("votes_Docks v1")
    val votesDocksV1: Int = 0

    @SerializedName("ToxicologistInventory")
    val toxicologistinventory: Map<String, Any> = Collections.emptyMap()

    val reaper: Int = 0

    val golem: Int = 0

    val farmer: Int = 0

    val florist: Int = 0

    @SerializedName("NecromancerInventory")
    val necromancerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("BakerInventory")
    val bakerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("HorsetamerInventory")
    val horsetamerinventory: Map<String, Any> = Collections.emptyMap()

    val snowman: Int = 0

    @SerializedName("inGamePresentsCap_2017_1")
    val ingamepresentscap20171: Int = 0

    @SerializedName("inGamePresentsCap_2017_2")
    val ingamepresentscap20172: Int = 0

    @SerializedName("inGamePresentsCap_2017_3")
    val ingamepresentscap20173: Int = 0

    @SerializedName("inGamePresentsCap_2017_4")
    val ingamepresentscap20174: Int = 0

    @SerializedName("inGamePresentsCap_2017_5")
    val ingamepresentscap20175: Int = 0

    @SerializedName("FloristInventory")
    val floristinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("inGamePresentsCap_2017_6")
    val ingamepresentscap20176: Int = 0

    @SerializedName("inGamePresentsCap_2017_7")
    val ingamepresentscap20177: Int = 0

    @SerializedName("inGamePresentsCap_2017_8")
    val ingamepresentscap20178: Int = 0

    @SerializedName("inGamePresentsCap_2017_9")
    val ingamepresentscap20179: Int = 0

    @SerializedName("inGamePresentsCap_2017_12")
    val ingamepresentscap201712: Int = 0

    @SerializedName("inGamePresentsCap_2017_13")
    val ingamepresentscap201713: Int = 0

    @SerializedName("inGamePresentsCap_2017_14")
    val ingamepresentscap201714: Int = 0

    @SerializedName("inGamePresentsCap_2017_15")
    val ingamepresentscap201715: Int = 0

    @SerializedName("inGamePresentsCap_2017_16")
    val ingamepresentscap201716: Int = 0

    @SerializedName("inGamePresentsCap_2017_17")
    val ingamepresentscap201717: Int = 0

    @SerializedName("inGamePresentsCap_2017_18")
    val ingamepresentscap201718: Int = 0

    @SerializedName("inGamePresentsCap_2017_19")
    val ingamepresentscap201719: Int = 0

    @SerializedName("inGamePresentsCap_2017_20")
    val ingamepresentscap201720: Int = 0

    @SerializedName("inGamePresentsCap_2017_21")
    val ingamepresentscap201721: Int = 0

    @SerializedName("inGamePresentsCap_2017_22")
    val ingamepresentscap201722: Int = 0

    @SerializedName("inGamePresentsCap_2017_23")
    val ingamepresentscap201723: Int = 0

    @SerializedName("inGamePresentsCap_2017_24")
    val ingamepresentscap201724: Int = 0

    @SerializedName("inGamePresentsCap_2017_27")
    val ingamepresentscap201727: Int = 0

    @SerializedName("inGamePresentsCap_2017_28")
    val ingamepresentscap201728: Int = 0

    @SerializedName("inGamePresentsCap_2017_29")
    val ingamepresentscap201729: Int = 0

    @SerializedName("inGamePresentsCap_2017_30")
    val ingamepresentscap201730: Int = 0

    @SerializedName("inGamePresentsCap_2017_31")
    val ingamepresentscap201731: Int = 0

    @SerializedName("inGamePresentsCap_2018_1")
    val ingamepresentscap20181: Int = 0

    @SerializedName("inGamePresentsCap_2018_2")
    val ingamepresentscap20182: Int = 0

    @SerializedName("inGamePresentsCap_2018_3")
    val ingamepresentscap20183: Int = 0

    @SerializedName("inGamePresentsCap_2018_4")
    val ingamepresentscap20184: Int = 0

    @SerializedName("inGamePresentsCap_2018_5")
    val ingamepresentscap20185: Int = 0

    @SerializedName("inGamePresentsCap_2018_6")
    val ingamepresentscap20186: Int = 0

    @SerializedName("inGamePresentsCap_2018_7")
    val ingamepresentscap20187: Int = 0

    @SerializedName("inGamePresentsCap_2018_8")
    val ingamepresentscap20188: Int = 0

    @SerializedName("SlimeySlimeInventory")
    val slimeyslimeinventory: Map<String, Any> = Collections.emptyMap()

    val autoarmor: Boolean = false

    @SerializedName("Shadow KnightInventory")
    val shadowKnightinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("RedDragonInventory")
    val reddragoninventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("RogueInventory")
    val rogueinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("taunt_kills")
    val tauntKills: Int = 0

    @SerializedName("tnt_placed_creepertamer")
    val tntPlacedCreepertamer: Int = 0

    @SerializedName("potions_drunk")
    val potionsDrunk: Int = 0

    val damage: Int = 0

    @SerializedName("mobs_spawned")
    val mobsSpawned: Int = 0

    @SerializedName("arrows_hit_creepertamer")
    val arrowsHitCreepertamer: Int = 0

    @SerializedName("damage_creepertamer")
    val damageCreepertamer: Int = 0

    @SerializedName("time_played")
    val timePlayed: Int = 0

    @SerializedName("taunt_kills_creepertamer")
    val tauntKillsCreepertamer: Int = 0

    @SerializedName("damage_taken_creepertamer")
    val damageTakenCreepertamer: Int = 0

    @SerializedName("kills_creepertamer")
    val killsCreepertamer: Int = 0

    @SerializedName("chests_opened")
    val chestsOpened: Int = 0

    @SerializedName("time_played_creepertamer")
    val timePlayedCreepertamer: Int = 0

    @SerializedName("games_played_creepertamer")
    val gamesPlayedCreepertamer: Int = 0

    @SerializedName("arrows_hit")
    val arrowsHit: Int = 0

    @SerializedName("wins_creepertamer")
    val winsCreepertamer: Int = 0

    @SerializedName("mobs_spawned_creepertamer")
    val mobsSpawnedCreepertamer: Int = 0

    @SerializedName("chests_opened_creepertamer")
    val chestsOpenedCreepertamer: Int = 0

    @SerializedName("damage_taken")
    val damageTaken: Int = 0

    @SerializedName("potions_drunk_creepertamer")
    val potionsDrunkCreepertamer: Int = 0

    @SerializedName("games_played")
    val gamesPlayed: Int = 0

    @SerializedName("time_played_baker")
    val timePlayedBaker: Int = 0

    @SerializedName("kills_baker")
    val killsBaker: Int = 0

    @SerializedName("potions_thrown")
    val potionsThrown: Int = 0

    @SerializedName("games_played_baker")
    val gamesPlayedBaker: Int = 0

    @SerializedName("chests_opened_baker")
    val chestsOpenedBaker: Int = 0

    @SerializedName("damage_taken_baker")
    val damageTakenBaker: Int = 0

    @SerializedName("potions_thrown_baker")
    val potionsThrownBaker: Int = 0

    @SerializedName("damage_baker")
    val damageBaker: Int = 0

    @SerializedName("potions_drunk_baker")
    val potionsDrunkBaker: Int = 0

    @SerializedName("wins_troll")
    val winsTroll: Int = 0

    @SerializedName("arrows_fired_troll")
    val arrowsFiredTroll: Int = 0

    @SerializedName("kills_troll")
    val killsTroll: Int = 0

    @SerializedName("mobs_spawned_troll")
    val mobsSpawnedTroll: Int = 0

    @SerializedName("potions_thrown_troll")
    val potionsThrownTroll: Int = 0

    @SerializedName("damage_taken_troll")
    val damageTakenTroll: Int = 0

    @SerializedName("arrows_hit_troll")
    val arrowsHitTroll: Int = 0

    @SerializedName("arrows_fired")
    val arrowsFired: Int = 0

    @SerializedName("games_played_troll")
    val gamesPlayedTroll: Int = 0

    @SerializedName("chests_opened_troll")
    val chestsOpenedTroll: Int = 0

    @SerializedName("time_played_troll")
    val timePlayedTroll: Int = 0

    @SerializedName("damage_troll")
    val damageTroll: Int = 0

    @SerializedName("potions_drunk_troll")
    val potionsDrunkTroll: Int = 0

    val guardian: Int = 0

    @SerializedName("wins_archer")
    val winsArcher: Int = 0

    @SerializedName("damage_taken_archer")
    val damageTakenArcher: Int = 0

    @SerializedName("arrows_fired_archer")
    val arrowsFiredArcher: Int = 0

    @SerializedName("taunt_kills_archer")
    val tauntKillsArcher: Int = 0

    @SerializedName("games_played_archer")
    val gamesPlayedArcher: Int = 0

    @SerializedName("potions_drunk_archer")
    val potionsDrunkArcher: Int = 0

    @SerializedName("kills_archer")
    val killsArcher: Int = 0

    @SerializedName("damage_archer")
    val damageArcher: Int = 0

    @SerializedName("mobs_spawned_archer")
    val mobsSpawnedArcher: Int = 0

    @SerializedName("time_played_archer")
    val timePlayedArcher: Int = 0

    @SerializedName("arrows_hit_archer")
    val arrowsHitArcher: Int = 0

    @SerializedName("chests_opened_archer")
    val chestsOpenedArcher: Int = 0

    @SerializedName("blitz_uses")
    val blitzUses: Int = 0

    @SerializedName("potions_thrown_archer")
    val potionsThrownArcher: Int = 0

    @SerializedName("mobs_spawned_slimeyslime")
    val mobsSpawnedSlimeyslime: Int = 0

    @SerializedName("games_played_slimeyslime")
    val gamesPlayedSlimeyslime: Int = 0

    @SerializedName("damage_slimeyslime")
    val damageSlimeyslime: Int = 0

    @SerializedName("damage_taken_slimeyslime")
    val damageTakenSlimeyslime: Int = 0

    @SerializedName("potions_thrown_slimeyslime")
    val potionsThrownSlimeyslime: Int = 0

    @SerializedName("chests_opened_slimeyslime")
    val chestsOpenedSlimeyslime: Int = 0

    @SerializedName("time_played_slimeyslime")
    val timePlayedSlimeyslime: Int = 0

    @SerializedName("potions_drunk_slimeyslime")
    val potionsDrunkSlimeyslime: Int = 0

    @SerializedName("kills_slimeyslime")
    val killsSlimeyslime: Int = 0

    @SerializedName("mobs_spawned_speleologist")
    val mobsSpawnedSpeleologist: Int = 0

    @SerializedName("wins_speleologist")
    val winsSpeleologist: Int = 0

    @SerializedName("games_played_speleologist")
    val gamesPlayedSpeleologist: Int = 0

    @SerializedName("time_played_speleologist")
    val timePlayedSpeleologist: Int = 0

    @SerializedName("potions_drunk_speleologist")
    val potionsDrunkSpeleologist: Int = 0

    @SerializedName("chests_opened_speleologist")
    val chestsOpenedSpeleologist: Int = 0

    @SerializedName("arrows_fired_speleologist")
    val arrowsFiredSpeleologist: Int = 0

    @SerializedName("kills_speleologist")
    val killsSpeleologist: Int = 0

    @SerializedName("damage_taken_speleologist")
    val damageTakenSpeleologist: Int = 0

    @SerializedName("arrows_hit_speleologist")
    val arrowsHitSpeleologist: Int = 0

    @SerializedName("damage_speleologist")
    val damageSpeleologist: Int = 0

    @SerializedName("MeatmasterInventory")
    val meatmasterinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("SpeleologistInventory")
    val speleologistinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("KnightInventory")
    val knightinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("damage_taken_guardian")
    val damageTakenGuardian: Int = 0

    @SerializedName("time_played_guardian")
    val timePlayedGuardian: Int = 0

    @SerializedName("damage_guardian")
    val damageGuardian: Int = 0

    @SerializedName("games_played_guardian")
    val gamesPlayedGuardian: Int = 0

    @SerializedName("chests_opened_guardian")
    val chestsOpenedGuardian: Int = 0

    @SerializedName("mobs_spawned_baker")
    val mobsSpawnedBaker: Int = 0

    @SerializedName("arrows_hit_baker")
    val arrowsHitBaker: Int = 0

    @SerializedName("arrows_fired_baker")
    val arrowsFiredBaker: Int = 0

    @SerializedName("wins_baker")
    val winsBaker: Int = 0

    @SerializedName("GuardianInventory")
    val guardianinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("Hype TrainInventory")
    val hypeTraininventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("ArmorerInventory")
    val armorerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("AstronautInventory")
    val astronautinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("ReaperInventory")
    val reaperinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("WarlockInventory")
    val warlockinventory: Map<String, Any> = Collections.emptyMap()

    val warlock: Int = 0

    @SerializedName("JockeyInventory")
    val jockeyinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("GolemInventory")
    val goleminventory: Map<String, Any> = Collections.emptyMap()

    val viking: Int = 0

    @SerializedName("VikingInventory")
    val vikinginventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("PigmanInventory")
    val pigmaninventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("PaladinInventory")
    val paladininventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("ArachnologistInventory")
    val arachnologistinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("BlazeInventory")
    val blazeinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("WolftamerInventory")
    val wolftamerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("TimInventory")
    val timinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("FarmerInventory")
    val farmerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("SnowmanInventory")
    val snowmaninventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("arrows_fired_necromancer")
    val arrowsFiredNecromancer: Int = 0

    @SerializedName("chests_opened_necromancer")
    val chestsOpenedNecromancer: Int = 0

    @SerializedName("mobs_spawned_necromancer")
    val mobsSpawnedNecromancer: Int = 0

    @SerializedName("damage_necromancer")
    val damageNecromancer: Int = 0

    @SerializedName("damage_taken_necromancer")
    val damageTakenNecromancer: Int = 0

    @SerializedName("games_played_necromancer")
    val gamesPlayedNecromancer: Int = 0

    @SerializedName("time_played_necromancer")
    val timePlayedNecromancer: Int = 0

    @SerializedName("kills_necromancer")
    val killsNecromancer: Int = 0

    @SerializedName("chests_opened_tim")
    val chestsOpenedTim: Int = 0

    @SerializedName("damage_taken_tim")
    val damageTakenTim: Int = 0

    @SerializedName("bottles_thrown_tim")
    val bottlesThrownTim: Int = 0

    @SerializedName("games_played_tim")
    val gamesPlayedTim: Int = 0

    @SerializedName("potions_drunk_tim")
    val potionsDrunkTim: Int = 0

    @SerializedName("kills_tim")
    val killsTim: Int = 0

    @SerializedName("damage_tim")
    val damageTim: Int = 0

    @SerializedName("time_played_tim")
    val timePlayedTim: Int = 0

    @SerializedName("arrows_hit_viking")
    val arrowsHitViking: Int = 0

    @SerializedName("chests_opened_viking")
    val chestsOpenedViking: Int = 0

    @SerializedName("damage_viking")
    val damageViking: Int = 0

    @SerializedName("arrows_fired_viking")
    val arrowsFiredViking: Int = 0

    @SerializedName("time_played_viking")
    val timePlayedViking: Int = 0

    @SerializedName("damage_taken_viking")
    val damageTakenViking: Int = 0

    @SerializedName("potions_drunk_viking")
    val potionsDrunkViking: Int = 0

    @SerializedName("games_played_viking")
    val gamesPlayedViking: Int = 0

    @SerializedName("wins_teams_viking")
    val winsTeamsViking: Int = 0

    @SerializedName("kills_viking")
    val killsViking: Int = 0

    @SerializedName("wins_teams_rogue")
    val winsTeamsRogue: Int = 0

    @SerializedName("fall_damage_rogue")
    val fallDamageRogue: Int = 0

    @SerializedName("potions_drunk_rogue")
    val potionsDrunkRogue: Int = 0

    @SerializedName("games_played_rogue")
    val gamesPlayedRogue: Int = 0

    @SerializedName("kills_rogue")
    val killsRogue: Int = 0

    @SerializedName("time_played_rogue")
    val timePlayedRogue: Int = 0

    @SerializedName("chests_opened_rogue")
    val chestsOpenedRogue: Int = 0

    @SerializedName("damage_rogue")
    val damageRogue: Int = 0

    @SerializedName("arrows_hit_rogue")
    val arrowsHitRogue: Int = 0

    @SerializedName("damage_taken_rogue")
    val damageTakenRogue: Int = 0

    @SerializedName("arrows_fired_rogue")
    val arrowsFiredRogue: Int = 0

    @SerializedName("kills_diver")
    val killsDiver: Int = 0

    @SerializedName("time_played_diver")
    val timePlayedDiver: Int = 0

    @SerializedName("mobs_spawned_diver")
    val mobsSpawnedDiver: Int = 0

    @SerializedName("chests_opened_diver")
    val chestsOpenedDiver: Int = 0

    @SerializedName("games_played_diver")
    val gamesPlayedDiver: Int = 0

    @SerializedName("damage_taken_diver")
    val damageTakenDiver: Int = 0

    @SerializedName("potions_drunk_diver")
    val potionsDrunkDiver: Int = 0

    @SerializedName("arrows_hit_diver")
    val arrowsHitDiver: Int = 0

    @SerializedName("damage_diver")
    val damageDiver: Int = 0

    @SerializedName("kills_paladin")
    val killsPaladin: Int = 0

    @SerializedName("damage_paladin")
    val damagePaladin: Int = 0

    @SerializedName("chests_opened_paladin")
    val chestsOpenedPaladin: Int = 0

    @SerializedName("time_played_paladin")
    val timePlayedPaladin: Int = 0

    @SerializedName("potions_drunk_paladin")
    val potionsDrunkPaladin: Int = 0

    @SerializedName("potions_thrown_paladin")
    val potionsThrownPaladin: Int = 0

    @SerializedName("games_played_paladin")
    val gamesPlayedPaladin: Int = 0

    @SerializedName("damage_taken_paladin")
    val damageTakenPaladin: Int = 0

    @SerializedName("wins_teams_slimeyslime")
    val winsTeamsSlimeyslime: Int = 0

    @SerializedName("arrows_fired_slimeyslime")
    val arrowsFiredSlimeyslime: Int = 0

    @SerializedName("arrows_hit_slimeyslime")
    val arrowsHitSlimeyslime: Int = 0

    val diver: Int = 0

    @SerializedName("mobs_spawned_rogue")
    val mobsSpawnedRogue: Int = 0

    @SerializedName("taunt_kills_rogue")
    val tauntKillsRogue: Int = 0

    @SerializedName("potions_thrown_rogue")
    val potionsThrownRogue: Int = 0

    @SerializedName("games_played_warlock")
    val gamesPlayedWarlock: Int = 0

    @SerializedName("kills_warlock")
    val killsWarlock: Int = 0

    @SerializedName("chests_opened_warlock")
    val chestsOpenedWarlock: Int = 0

    @SerializedName("damage_taken_warlock")
    val damageTakenWarlock: Int = 0

    @SerializedName("time_played_warlock")
    val timePlayedWarlock: Int = 0

    @SerializedName("potions_thrown_warlock")
    val potionsThrownWarlock: Int = 0

    @SerializedName("damage_warlock")
    val damageWarlock: Int = 0

    @SerializedName("potions_drunk_warlock")
    val potionsDrunkWarlock: Int = 0

    @SerializedName("damage_taken_fisherman")
    val damageTakenFisherman: Int = 0

    @SerializedName("potions_drunk_fisherman")
    val potionsDrunkFisherman: Int = 0

    @SerializedName("games_played_fisherman")
    val gamesPlayedFisherman: Int = 0

    @SerializedName("chests_opened_fisherman")
    val chestsOpenedFisherman: Int = 0

    @SerializedName("kills_fisherman")
    val killsFisherman: Int = 0

    @SerializedName("damage_fisherman")
    val damageFisherman: Int = 0

    @SerializedName("arrows_hit_fisherman")
    val arrowsHitFisherman: Int = 0

    @SerializedName("arrows_fired_fisherman")
    val arrowsFiredFisherman: Int = 0

    @SerializedName("time_played_fisherman")
    val timePlayedFisherman: Int = 0

    @SerializedName("potions_thrown_viking")
    val potionsThrownViking: Int = 0

    @SerializedName("DiverInventory")
    val diverinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("kills_florist")
    val killsFlorist: Int = 0

    @SerializedName("damage_florist")
    val damageFlorist: Int = 0

    @SerializedName("games_played_florist")
    val gamesPlayedFlorist: Int = 0

    @SerializedName("potions_thrown_florist")
    val potionsThrownFlorist: Int = 0

    @SerializedName("time_played_florist")
    val timePlayedFlorist: Int = 0

    @SerializedName("chests_opened_florist")
    val chestsOpenedFlorist: Int = 0

    @SerializedName("damage_taken_florist")
    val damageTakenFlorist: Int = 0

    @SerializedName("wins_teams_baker")
    val winsTeamsBaker: Int = 0

    @SerializedName("potions_thrown_speleologist")
    val potionsThrownSpeleologist: Int = 0

    @SerializedName("mobs_spawned_pigman")
    val mobsSpawnedPigman: Int = 0

    @SerializedName("arrows_fired_pigman")
    val arrowsFiredPigman: Int = 0

    @SerializedName("damage_pigman")
    val damagePigman: Int = 0

    @SerializedName("damage_taken_pigman")
    val damageTakenPigman: Int = 0

    @SerializedName("wins_teams_pigman")
    val winsTeamsPigman: Int = 0

    @SerializedName("arrows_hit_pigman")
    val arrowsHitPigman: Int = 0

    @SerializedName("chests_opened_pigman")
    val chestsOpenedPigman: Int = 0

    @SerializedName("time_played_pigman")
    val timePlayedPigman: Int = 0

    @SerializedName("potions_thrown_pigman")
    val potionsThrownPigman: Int = 0

    @SerializedName("potions_drunk_pigman")
    val potionsDrunkPigman: Int = 0

    @SerializedName("games_played_pigman")
    val gamesPlayedPigman: Int = 0

    @SerializedName("kills_pigman")
    val killsPigman: Int = 0

    @SerializedName("kills_golem")
    val killsGolem: Int = 0

    @SerializedName("arrows_fired_golem")
    val arrowsFiredGolem: Int = 0

    @SerializedName("damage_taken_golem")
    val damageTakenGolem: Int = 0

    @SerializedName("arrows_hit_golem")
    val arrowsHitGolem: Int = 0

    @SerializedName("taunt_kills_golem")
    val tauntKillsGolem: Int = 0

    @SerializedName("chests_opened_golem")
    val chestsOpenedGolem: Int = 0

    @SerializedName("damage_golem")
    val damageGolem: Int = 0

    @SerializedName("games_played_golem")
    val gamesPlayedGolem: Int = 0

    @SerializedName("potions_thrown_golem")
    val potionsThrownGolem: Int = 0

    @SerializedName("wins_golem")
    val winsGolem: Int = 0

    @SerializedName("potions_drunk_golem")
    val potionsDrunkGolem: Int = 0

    @SerializedName("time_played_golem")
    val timePlayedGolem: Int = 0

    @SerializedName("potions_thrown_tim")
    val potionsThrownTim: Int = 0

    @SerializedName("arrows_fired_tim")
    val arrowsFiredTim: Int = 0

    @SerializedName("wins_tim")
    val winsTim: Int = 0

    @SerializedName("arrows_hit_tim")
    val arrowsHitTim: Int = 0

    @SerializedName("mobs_spawned_tim")
    val mobsSpawnedTim: Int = 0

    @SerializedName("items_enchanted_tim")
    val itemsEnchantedTim: Int = 0

    @SerializedName("potions_thrown_diver")
    val potionsThrownDiver: Int = 0

    @SerializedName("arrows_fired_diver")
    val arrowsFiredDiver: Int = 0

    @SerializedName("wins_diver")
    val winsDiver: Int = 0

    @SerializedName("taunt_kills_diver")
    val tauntKillsDiver: Int = 0

    @SerializedName("wins_teams_golem")
    val winsTeamsGolem: Int = 0

    @SerializedName("chests_opened_horsetamer")
    val chestsOpenedHorsetamer: Int = 0

    @SerializedName("blocks_traveled_horse_horsetamer")
    val blocksTraveledHorseHorsetamer: Int = 0

    @SerializedName("games_played_horsetamer")
    val gamesPlayedHorsetamer: Int = 0

    @SerializedName("kills_horsetamer")
    val killsHorsetamer: Int = 0

    @SerializedName("time_played_horsetamer")
    val timePlayedHorsetamer: Int = 0

    @SerializedName("potions_drunk_horsetamer")
    val potionsDrunkHorsetamer: Int = 0

    @SerializedName("arrows_fired_horsetamer")
    val arrowsFiredHorsetamer: Int = 0

    @SerializedName("damage_horsetamer")
    val damageHorsetamer: Int = 0

    @SerializedName("arrows_hit_horsetamer")
    val arrowsHitHorsetamer: Int = 0

    @SerializedName("damage_taken_horsetamer")
    val damageTakenHorsetamer: Int = 0

    @SerializedName("mobs_spawned_horsetamer")
    val mobsSpawnedHorsetamer: Int = 0

    @SerializedName("mobs_spawned_golem")
    val mobsSpawnedGolem: Int = 0

    @SerializedName("wins_teams_archer")
    val winsTeamsArcher: Int = 0

    @SerializedName("fall_damage_astronaut")
    val fallDamageAstronaut: Int = 0

    @SerializedName("damage_astronaut")
    val damageAstronaut: Int = 0

    @SerializedName("time_played_astronaut")
    val timePlayedAstronaut: Int = 0

    @SerializedName("arrows_fired_astronaut")
    val arrowsFiredAstronaut: Int = 0

    @SerializedName("games_played_astronaut")
    val gamesPlayedAstronaut: Int = 0

    @SerializedName("potions_drunk_astronaut")
    val potionsDrunkAstronaut: Int = 0

    @SerializedName("chests_opened_astronaut")
    val chestsOpenedAstronaut: Int = 0

    @SerializedName("kills_astronaut")
    val killsAstronaut: Int = 0

    @SerializedName("arrows_hit_astronaut")
    val arrowsHitAstronaut: Int = 0

    @SerializedName("damage_taken_astronaut")
    val damageTakenAstronaut: Int = 0

    @SerializedName("taunt_kills_astronaut")
    val tauntKillsAstronaut: Int = 0

    @SerializedName("wins_teams_speleologist")
    val winsTeamsSpeleologist: Int = 0

    @SerializedName("taunt_kills_speleologist")
    val tauntKillsSpeleologist: Int = 0

    @SerializedName("wins_teams_astronaut")
    val winsTeamsAstronaut: Int = 0

    @SerializedName("taunt_kills_slimeyslime")
    val tauntKillsSlimeyslime: Int = 0

    @SerializedName("wins_slimeyslime")
    val winsSlimeyslime: Int = 0

    @SerializedName("arrows_fired_florist")
    val arrowsFiredFlorist: Int = 0

    @SerializedName("potions_drunk_florist")
    val potionsDrunkFlorist: Int = 0

    @SerializedName("arrows_hit_florist")
    val arrowsHitFlorist: Int = 0

    @SerializedName("wins_paladin")
    val winsPaladin: Int = 0

    @SerializedName("arrows_fired_paladin")
    val arrowsFiredPaladin: Int = 0

    @SerializedName("wins_teams_diver")
    val winsTeamsDiver: Int = 0

    @SerializedName("wins_pigman")
    val winsPigman: Int = 0

    @SerializedName("wins_horsetamer")
    val winsHorsetamer: Int = 0

    @SerializedName("taunt_kills_troll")
    val tauntKillsTroll: Int = 0

    @SerializedName("arrows_fired_creepertamer")
    val arrowsFiredCreepertamer: Int = 0

    @SerializedName("potions_drunk_random")
    val potionsDrunkRandom: Int = 0

    @SerializedName("mobs_spawned_random")
    val mobsSpawnedRandom: Int = 0

    @SerializedName("arrows_hit_random")
    val arrowsHitRandom: Int = 0

    @SerializedName("kills_random")
    val killsRandom: Int = 0

    @SerializedName("damage_taken_random")
    val damageTakenRandom: Int = 0

    @SerializedName("games_played_random")
    val gamesPlayedRandom: Int = 0

    @SerializedName("arrows_fired_random")
    val arrowsFiredRandom: Int = 0

    @SerializedName("time_played_random")
    val timePlayedRandom: Int = 0

    @SerializedName("chests_opened_random")
    val chestsOpenedRandom: Int = 0

    @SerializedName("damage_random")
    val damageRandom: Int = 0

    @SerializedName("random_wins")
    val randomWins: Int = 0

    @SerializedName("taunt_kills_random")
    val tauntKillsRandom: Int = 0

    @SerializedName("wins_random")
    val winsRandom: Int = 0

    @SerializedName("potions_thrown_random")
    val potionsThrownRandom: Int = 0

    @SerializedName("wins_florist")
    val winsFlorist: Int = 0

    @SerializedName("taunt_kills_pigman")
    val tauntKillsPigman: Int = 0

    @SerializedName("mobs_spawned_florist")
    val mobsSpawnedFlorist: Int = 0

    @SerializedName("potions_thrown_horsetamer")
    val potionsThrownHorsetamer: Int = 0

    @SerializedName("explosive_kills_creepertamer")
    val explosiveKillsCreepertamer: Int = 0

    @SerializedName("games_played_armorer")
    val gamesPlayedArmorer: Int = 0

    @SerializedName("damage_taken_armorer")
    val damageTakenArmorer: Int = 0

    @SerializedName("chests_opened_armorer")
    val chestsOpenedArmorer: Int = 0

    @SerializedName("time_played_armorer")
    val timePlayedArmorer: Int = 0

    @SerializedName("potions_drunk_armorer")
    val potionsDrunkArmorer: Int = 0

    @SerializedName("damage_armorer")
    val damageArmorer: Int = 0

    @SerializedName("kills_armorer")
    val killsArmorer: Int = 0

    @SerializedName("mobs_spawned_fisherman")
    val mobsSpawnedFisherman: Int = 0

    @SerializedName("potions_thrown_fisherman")
    val potionsThrownFisherman: Int = 0

    @SerializedName("chests_opened_meatmaster")
    val chestsOpenedMeatmaster: Int = 0

    @SerializedName("mobs_spawned_meatmaster")
    val mobsSpawnedMeatmaster: Int = 0

    @SerializedName("arrows_fired_meatmaster")
    val arrowsFiredMeatmaster: Int = 0

    @SerializedName("wins_meatmaster")
    val winsMeatmaster: Int = 0

    @SerializedName("damage_taken_meatmaster")
    val damageTakenMeatmaster: Int = 0

    @SerializedName("games_played_meatmaster")
    val gamesPlayedMeatmaster: Int = 0

    @SerializedName("arrows_hit_meatmaster")
    val arrowsHitMeatmaster: Int = 0

    @SerializedName("damage_meatmaster")
    val damageMeatmaster: Int = 0

    @SerializedName("potions_drunk_meatmaster")
    val potionsDrunkMeatmaster: Int = 0

    @SerializedName("time_played_meatmaster")
    val timePlayedMeatmaster: Int = 0

    @SerializedName("kills_meatmaster")
    val killsMeatmaster: Int = 0

    @SerializedName("wins_teams_creepertamer")
    val winsTeamsCreepertamer: Int = 0

    @SerializedName("wins_armorer")
    val winsArmorer: Int = 0

    @SerializedName("potions_thrown_armorer")
    val potionsThrownArmorer: Int = 0

    @SerializedName("arrows_fired_armorer")
    val arrowsFiredArmorer: Int = 0

    @SerializedName("mobs_spawned_armorer")
    val mobsSpawnedArmorer: Int = 0

    @SerializedName("potions_thrown_creepertamer")
    val potionsThrownCreepertamer: Int = 0

    @SerializedName("kills_hunter")
    val killsHunter: Int = 0

    @SerializedName("damage_hunter")
    val damageHunter: Int = 0

    @SerializedName("mobs_spawned_hunter")
    val mobsSpawnedHunter: Int = 0

    @SerializedName("games_played_hunter")
    val gamesPlayedHunter: Int = 0

    @SerializedName("time_played_hunter")
    val timePlayedHunter: Int = 0

    @SerializedName("arrows_hit_hunter")
    val arrowsHitHunter: Int = 0

    @SerializedName("chests_opened_hunter")
    val chestsOpenedHunter: Int = 0

    @SerializedName("damage_taken_hunter")
    val damageTakenHunter: Int = 0

    @SerializedName("arrows_fired_hunter")
    val arrowsFiredHunter: Int = 0

    @SerializedName("wins_hunter")
    val winsHunter: Int = 0

    @SerializedName("potions_thrown_hunter")
    val potionsThrownHunter: Int = 0

    @SerializedName("potions_drunk_hunter")
    val potionsDrunkHunter: Int = 0

    @SerializedName("kills_reaper")
    val killsReaper: Int = 0

    @SerializedName("damage_reaper")
    val damageReaper: Int = 0

    @SerializedName("potions_drunk_reaper")
    val potionsDrunkReaper: Int = 0

    @SerializedName("damage_taken_reaper")
    val damageTakenReaper: Int = 0

    @SerializedName("games_played_reaper")
    val gamesPlayedReaper: Int = 0

    @SerializedName("chests_opened_reaper")
    val chestsOpenedReaper: Int = 0

    @SerializedName("time_played_reaper")
    val timePlayedReaper: Int = 0

    @SerializedName("wins_necromancer")
    val winsNecromancer: Int = 0

    @SerializedName("taunt_kills_necromancer")
    val tauntKillsNecromancer: Int = 0

    @SerializedName("potions_drunk_necromancer")
    val potionsDrunkNecromancer: Int = 0

    @SerializedName("arrows_hit_necromancer")
    val arrowsHitNecromancer: Int = 0

    @SerializedName("potions_thrown_necromancer")
    val potionsThrownNecromancer: Int = 0

    @SerializedName("taunt_kills_baker")
    val tauntKillsBaker: Int = 0

    @SerializedName("wins_teams_random")
    val winsTeamsRandom: Int = 0

    @SerializedName("wins_teams_meatmaster")
    val winsTeamsMeatmaster: Int = 0

    @SerializedName("blocks_traveled_pig_meatmaster")
    val blocksTraveledPigMeatmaster: Int = 0

    @SerializedName("wins_teams_hunter")
    val winsTeamsHunter: Int = 0

    @SerializedName("taunt_kills_horsetamer")
    val tauntKillsHorsetamer: Int = 0

    @SerializedName("wins_teams_horsetamer")
    val winsTeamsHorsetamer: Int = 0

    @SerializedName("wins_teams_troll")
    val winsTeamsTroll: Int = 0

    @SerializedName("damage_taken_scout")
    val damageTakenScout: Int = 0

    @SerializedName("potions_thrown_scout")
    val potionsThrownScout: Int = 0

    @SerializedName("arrows_hit_scout")
    val arrowsHitScout: Int = 0

    @SerializedName("potions_drunk_scout")
    val potionsDrunkScout: Int = 0

    @SerializedName("damage_scout")
    val damageScout: Int = 0

    @SerializedName("kills_scout")
    val killsScout: Int = 0

    @SerializedName("time_played_scout")
    val timePlayedScout: Int = 0

    @SerializedName("chests_opened_scout")
    val chestsOpenedScout: Int = 0

    @SerializedName("games_played_scout")
    val gamesPlayedScout: Int = 0

    @SerializedName("wins_teams_scout")
    val winsTeamsScout: Int = 0

    @SerializedName("wins_teams_tim")
    val winsTeamsTim: Int = 0

    @SerializedName("taunt_kills_florist")
    val tauntKillsFlorist: Int = 0

    @SerializedName("chests_opened_hype train")
    val chestsOpenedHypeTrain: Int = 0

    @SerializedName("potions_drunk_hype train")
    val potionsDrunkHypeTrain: Int = 0

    @SerializedName("time_played_hype train")
    val timePlayedHypeTrain: Int = 0

    @SerializedName("games_played_hype train")
    val gamesPlayedHypeTrain: Int = 0

    @SerializedName("wins_teams_hype train")
    val winsTeamsHypeTrain: Int = 0

    @SerializedName("damage_taken_hype train")
    val damageTakenHypeTrain: Int = 0

    @SerializedName("damage_hype train")
    val damageHypeTrain: Int = 0

    @SerializedName("kills_hype train")
    val killsHypeTrain: Int = 0

    @SerializedName("potions_thrown_hype train")
    val potionsThrownHypeTrain: Int = 0

    @SerializedName("arrows_hit_hype train")
    val arrowsHitHypeTrain: Int = 0

    @SerializedName("arrows_fired_hype train")
    val arrowsFiredHypeTrain: Int = 0

    @SerializedName("time_played_shadow knight")
    val timePlayedShadowKnight: Int = 0

    @SerializedName("chests_opened_shadow knight")
    val chestsOpenedShadowKnight: Int = 0

    @SerializedName("arrows_hit_shadow knight")
    val arrowsHitShadowKnight: Int = 0

    @SerializedName("kills_shadow knight")
    val killsShadowKnight: Int = 0

    @SerializedName("damage_taken_shadow knight")
    val damageTakenShadowKnight: Int = 0

    @SerializedName("wins_teams_shadow knight")
    val winsTeamsShadowKnight: Int = 0

    @SerializedName("arrows_fired_shadow knight")
    val arrowsFiredShadowKnight: Int = 0

    @SerializedName("games_played_shadow knight")
    val gamesPlayedShadowKnight: Int = 0

    @SerializedName("potions_thrown_shadow knight")
    val potionsThrownShadowKnight: Int = 0

    @SerializedName("damage_shadow knight")
    val damageShadowKnight: Int = 0

    @SerializedName("potions_drunk_shadow knight")
    val potionsDrunkShadowKnight: Int = 0

    @SerializedName("wins_teams_florist")
    val winsTeamsFlorist: Int = 0

    @SerializedName("taunt_kills_meatmaster")
    val tauntKillsMeatmaster: Int = 0

    @SerializedName("potions_thrown_meatmaster")
    val potionsThrownMeatmaster: Int = 0

    @SerializedName("potions_thrown_astronaut")
    val potionsThrownAstronaut: Int = 0

    @SerializedName("chests_opened_snowman")
    val chestsOpenedSnowman: Int = 0

    @SerializedName("snowballs_thrown_snowman")
    val snowballsThrownSnowman: Int = 0

    @SerializedName("potions_drunk_snowman")
    val potionsDrunkSnowman: Int = 0

    @SerializedName("time_played_snowman")
    val timePlayedSnowman: Int = 0

    @SerializedName("damage_snowman")
    val damageSnowman: Int = 0

    @SerializedName("damage_taken_snowman")
    val damageTakenSnowman: Int = 0

    @SerializedName("games_played_snowman")
    val gamesPlayedSnowman: Int = 0

    @SerializedName("arrows_hit_snowman")
    val arrowsHitSnowman: Int = 0

    @SerializedName("arrows_fired_snowman")
    val arrowsFiredSnowman: Int = 0

    @SerializedName("kills_snowman")
    val killsSnowman: Int = 0

    @SerializedName("taunt_kills_armorer")
    val tauntKillsArmorer: Int = 0

    @SerializedName("wins_teams_armorer")
    val winsTeamsArmorer: Int = 0

    @SerializedName("arrows_hit_armorer")
    val arrowsHitArmorer: Int = 0

    @SerializedName("arrows_hit_paladin")
    val arrowsHitPaladin: Int = 0

    @SerializedName("kills_blaze")
    val killsBlaze: Int = 0

    @SerializedName("damage_blaze")
    val damageBlaze: Int = 0

    @SerializedName("chests_opened_blaze")
    val chestsOpenedBlaze: Int = 0

    @SerializedName("potions_drunk_blaze")
    val potionsDrunkBlaze: Int = 0

    @SerializedName("arrows_hit_blaze")
    val arrowsHitBlaze: Int = 0

    @SerializedName("time_played_blaze")
    val timePlayedBlaze: Int = 0

    @SerializedName("damage_taken_blaze")
    val damageTakenBlaze: Int = 0

    @SerializedName("games_played_blaze")
    val gamesPlayedBlaze: Int = 0

    @SerializedName("mobs_spawned_blaze")
    val mobsSpawnedBlaze: Int = 0

    @SerializedName("arrows_fired_blaze")
    val arrowsFiredBlaze: Int = 0

    @SerializedName("kills_guardian")
    val killsGuardian: Int = 0

    @SerializedName("potions_drunk_guardian")
    val potionsDrunkGuardian: Int = 0

    @SerializedName("time_played_rambo")
    val timePlayedRambo: Int = 0

    @SerializedName("damage_taken_rambo")
    val damageTakenRambo: Int = 0

    @SerializedName("damage_rambo")
    val damageRambo: Int = 0

    @SerializedName("games_played_rambo")
    val gamesPlayedRambo: Int = 0

    @SerializedName("chests_opened_rambo")
    val chestsOpenedRambo: Int = 0

    @SerializedName("arrows_fired_reaper")
    val arrowsFiredReaper: Int = 0

    @SerializedName("arrows_fired_scout")
    val arrowsFiredScout: Int = 0

    @SerializedName("wins_teams_blaze")
    val winsTeamsBlaze: Int = 0

    @SerializedName("votes_Pixelville")
    val votesPixelville: Int = 0

    @SerializedName("kills_rambo")
    val killsRambo: Int = 0

    @SerializedName("wins_rogue")
    val winsRogue: Int = 0

    @SerializedName("wins_reaper")
    val winsReaper: Int = 0

    @SerializedName("arrows_hit_reaper")
    val arrowsHitReaper: Int = 0

    @SerializedName("potions_thrown_reaper")
    val potionsThrownReaper: Int = 0

    @SerializedName("taunt_kills_viking")
    val tauntKillsViking: Int = 0

    @SerializedName("mobs_spawned_astronaut")
    val mobsSpawnedAstronaut: Int = 0

    @SerializedName("taunt_kills_wolftamer")
    val tauntKillsWolftamer: Int = 0

    @SerializedName("damage_taken_wolftamer")
    val damageTakenWolftamer: Int = 0

    @SerializedName("potions_drunk_wolftamer")
    val potionsDrunkWolftamer: Int = 0

    @SerializedName("time_played_wolftamer")
    val timePlayedWolftamer: Int = 0

    @SerializedName("wins_wolftamer")
    val winsWolftamer: Int = 0

    @SerializedName("chests_opened_wolftamer")
    val chestsOpenedWolftamer: Int = 0

    @SerializedName("games_played_wolftamer")
    val gamesPlayedWolftamer: Int = 0

    @SerializedName("kills_wolftamer")
    val killsWolftamer: Int = 0

    @SerializedName("damage_wolftamer")
    val damageWolftamer: Int = 0

    @SerializedName("potions_thrown_wolftamer")
    val potionsThrownWolftamer: Int = 0

    @SerializedName("arrows_fired_wolftamer")
    val arrowsFiredWolftamer: Int = 0

    @SerializedName("mobs_spawned_wolftamer")
    val mobsSpawnedWolftamer: Int = 0

    @SerializedName("blocks_traveled_minecart_hype train")
    val blocksTraveledMinecartHypeTrain: Int = 0

    @SerializedName("rails_placed_hype train")
    val railsPlacedHypeTrain: Int = 0

    @SerializedName("eggs_collected_farmer")
    val eggsCollectedFarmer: Int = 0

    @SerializedName("kills_farmer")
    val killsFarmer: Int = 0

    @SerializedName("damage_farmer")
    val damageFarmer: Int = 0

    @SerializedName("damage_taken_farmer")
    val damageTakenFarmer: Int = 0

    @SerializedName("mobs_spawned_farmer")
    val mobsSpawnedFarmer: Int = 0

    @SerializedName("time_played_farmer")
    val timePlayedFarmer: Int = 0

    @SerializedName("eggs_thrown_farmer")
    val eggsThrownFarmer: Int = 0

    @SerializedName("chests_opened_farmer")
    val chestsOpenedFarmer: Int = 0

    @SerializedName("games_played_farmer")
    val gamesPlayedFarmer: Int = 0

    @SerializedName("potions_drunk_farmer")
    val potionsDrunkFarmer: Int = 0

    @SerializedName("wins_teams_farmer")
    val winsTeamsFarmer: Int = 0

    @SerializedName("mobs_spawned_paladin")
    val mobsSpawnedPaladin: Int = 0

    @SerializedName("wins_teams_paladin")
    val winsTeamsPaladin: Int = 0

    @SerializedName("wins_scout")
    val winsScout: Int = 0

    @SerializedName("taunt_kills_scout")
    val tauntKillsScout: Int = 0

    @SerializedName("taunt_kills_hunter")
    val tauntKillsHunter: Int = 0

    @SerializedName("wins_hype train")
    val winsHypeTrain: Int = 0

    @SerializedName("taunt_kills_reaper")
    val tauntKillsReaper: Int = 0

    @SerializedName("chests_opened_knight")
    val chestsOpenedKnight: Int = 0

    @SerializedName("time_played_knight")
    val timePlayedKnight: Int = 0

    @SerializedName("damage_knight")
    val damageKnight: Int = 0

    @SerializedName("damage_taken_knight")
    val damageTakenKnight: Int = 0

    @SerializedName("potions_drunk_knight")
    val potionsDrunkKnight: Int = 0

    @SerializedName("games_played_knight")
    val gamesPlayedKnight: Int = 0

    @SerializedName("kills_knight")
    val killsKnight: Int = 0

    @SerializedName("arrows_hit_knight")
    val arrowsHitKnight: Int = 0

    @SerializedName("arrows_fired_guardian")
    val arrowsFiredGuardian: Int = 0

    @SerializedName("mobs_spawned_hype train")
    val mobsSpawnedHypeTrain: Int = 0

    @SerializedName("wins_astronaut")
    val winsAstronaut: Int = 0

    @SerializedName("mobs_spawned_warlock")
    val mobsSpawnedWarlock: Int = 0

    @SerializedName("arrows_fired_warlock")
    val arrowsFiredWarlock: Int = 0

    @SerializedName("chests_opened_jockey")
    val chestsOpenedJockey: Int = 0

    @SerializedName("arrows_hit_jockey")
    val arrowsHitJockey: Int = 0

    @SerializedName("games_played_jockey")
    val gamesPlayedJockey: Int = 0

    @SerializedName("potions_drunk_jockey")
    val potionsDrunkJockey: Int = 0

    @SerializedName("damage_taken_jockey")
    val damageTakenJockey: Int = 0

    @SerializedName("kills_jockey")
    val killsJockey: Int = 0

    @SerializedName("taunt_kills_jockey")
    val tauntKillsJockey: Int = 0

    @SerializedName("damage_jockey")
    val damageJockey: Int = 0

    @SerializedName("arrows_fired_jockey")
    val arrowsFiredJockey: Int = 0

    @SerializedName("time_played_jockey")
    val timePlayedJockey: Int = 0

    @SerializedName("mobs_spawned_viking")
    val mobsSpawnedViking: Int = 0

    @SerializedName("wins_viking")
    val winsViking: Int = 0

    @SerializedName("blocks_traveled_boat_viking")
    val blocksTraveledBoatViking: Int = 0

    @SerializedName("wins_shadow knight")
    val winsShadowKnight: Int = 0

    @SerializedName("taunt_kills_paladin")
    val tauntKillsPaladin: Int = 0

    @SerializedName("potions_thrown_blaze")
    val potionsThrownBlaze: Int = 0

    @SerializedName("wins_blaze")
    val winsBlaze: Int = 0

    @SerializedName("arrows_fired_farmer")
    val arrowsFiredFarmer: Int = 0

    @SerializedName("arrows_hit_farmer")
    val arrowsHitFarmer: Int = 0

    @SerializedName("potions_drunk_reddragon")
    val potionsDrunkReddragon: Int = 0

    @SerializedName("games_played_reddragon")
    val gamesPlayedReddragon: Int = 0

    @SerializedName("damage_reddragon")
    val damageReddragon: Int = 0

    @SerializedName("damage_taken_reddragon")
    val damageTakenReddragon: Int = 0

    @SerializedName("chests_opened_reddragon")
    val chestsOpenedReddragon: Int = 0

    @SerializedName("arrows_hit_reddragon")
    val arrowsHitReddragon: Int = 0

    @SerializedName("arrows_fired_reddragon")
    val arrowsFiredReddragon: Int = 0

    @SerializedName("time_played_reddragon")
    val timePlayedReddragon: Int = 0

    @SerializedName("kills_reddragon")
    val killsReddragon: Int = 0

    @SerializedName("wins_teams_necromancer")
    val winsTeamsNecromancer: Int = 0

    @SerializedName("potions_thrown_knight")
    val potionsThrownKnight: Int = 0

    @SerializedName("wins_teams_knight")
    val winsTeamsKnight: Int = 0

    @SerializedName("taunt_kills_guardian")
    val tauntKillsGuardian: Int = 0

    @SerializedName("wins_teams_guardian")
    val winsTeamsGuardian: Int = 0

    @SerializedName("taunt_kills_fisherman")
    val tauntKillsFisherman: Int = 0

    @SerializedName("wins_teams_fisherman")
    val winsTeamsFisherman: Int = 0

    @SerializedName("wins_fisherman")
    val winsFisherman: Int = 0

    @SerializedName("arrows_fired_knight")
    val arrowsFiredKnight: Int = 0

    @SerializedName("kit_item_rename_Golem_iron_axe")
    val kitItemRenameGolemIronAxe: String = ""

    @SerializedName("potions_thrown_guardian")
    val potionsThrownGuardian: Int = 0

    @SerializedName("tnt_placed_hype train")
    val tntPlacedHypeTrain: Int = 0

    @SerializedName("potions_drunk_rambo")
    val potionsDrunkRambo: Int = 0

    @SerializedName("mobs_spawned_snowman")
    val mobsSpawnedSnowman: Int = 0

    @SerializedName("arrows_fired_rambo")
    val arrowsFiredRambo: Int = 0

    @SerializedName("wins_rambo")
    val winsRambo: Int = 0

    @SerializedName("arrows_hit_rambo")
    val arrowsHitRambo: Int = 0

    @SerializedName("inGamePresentsCap_2018_9")
    val ingamepresentscap20189: Int = 0

    @SerializedName("inGamePresentsCap_2018_10")
    val ingamepresentscap201810: Int = 0

    @SerializedName("inGamePresentsCap_2018_11")
    val ingamepresentscap201811: Int = 0

    @SerializedName("inGamePresentsCap_2018_12")
    val ingamepresentscap201812: Int = 0

    @SerializedName("inGamePresentsCap_2018_13")
    val ingamepresentscap201813: Int = 0

    @SerializedName("inGamePresentsCap_2018_14")
    val ingamepresentscap201814: Int = 0

    @SerializedName("inGamePresentsCap_2018_15")
    val ingamepresentscap201815: Int = 0

    @SerializedName("inGamePresentsCap_2018_16")
    val ingamepresentscap201816: Int = 0

    @SerializedName("inGamePresentsCap_2018_17")
    val ingamepresentscap201817: Int = 0

    @SerializedName("inGamePresentsCap_2018_18")
    val ingamepresentscap201818: Int = 0

    @SerializedName("inGamePresentsCap_2018_20")
    val ingamepresentscap201820: Int = 0

    @SerializedName("inGamePresentsCap_2018_21")
    val ingamepresentscap201821: Int = 0

    val pcreepertamer: Int = 0

    @SerializedName("kit_item_rename_Creepertamer_leather_leggings")
    val kitItemRenameCreepertamerLeatherLeggings: String = ""

    @SerializedName("kit_item_rename_Creepertamer_stone_sword")
    val kitItemRenameCreepertamerStoneSword: String = ""

    @SerializedName("kit_item_rename_Creepertamer_diamond_chestplate")
    val kitItemRenameCreepertamerDiamondChestplate: String = ""

    @SerializedName("inGamePresentsCap_2018_22")
    val ingamepresentscap201822: Int = 0

    @SerializedName("chests_opened_toxicologist")
    val chestsOpenedToxicologist: Int = 0

    @SerializedName("damage_toxicologist")
    val damageToxicologist: Int = 0

    @SerializedName("time_played_toxicologist")
    val timePlayedToxicologist: Int = 0

    @SerializedName("damage_taken_toxicologist")
    val damageTakenToxicologist: Int = 0

    @SerializedName("potions_thrown_toxicologist")
    val potionsThrownToxicologist: Int = 0

    @SerializedName("games_played_toxicologist")
    val gamesPlayedToxicologist: Int = 0

    @SerializedName("inGamePresentsCap_2018_23")
    val ingamepresentscap201823: Int = 0

    @SerializedName("inGamePresentsCap_2018_26")
    val ingamepresentscap201826: Int = 0

    @SerializedName("inGamePresentsCap_2018_27")
    val ingamepresentscap201827: Int = 0

    @SerializedName("inGamePresentsCap_2018_28")
    val ingamepresentscap201828: Int = 0

    @SerializedName("inGamePresentsCap_2018_29")
    val ingamepresentscap201829: Int = 0

    @SerializedName("inGamePresentsCap_2018_30")
    val ingamepresentscap201830: Int = 0

    @SerializedName("inGamePresentsCap_2018_31")
    val ingamepresentscap201831: Int = 0

    @SerializedName("inGamePresentsCap_2019_2")
    val ingamepresentscap20192: Int = 0

    @SerializedName("inGamePresentsCap_2019_3")
    val ingamepresentscap20193: Int = 0

    @SerializedName("wins_solo_normal")
    val winsSoloNormal: Int = 0

    @SerializedName("wins_teams_normal")
    val winsTeamsNormal: Int = 0

    @SerializedName("wins_backup")
    val winsBackup: Int = 0

    @SerializedName("kills_solo_normal")
    val killsSoloNormal: Int = 0

    @SerializedName("games_played_phoenix")
    val gamesPlayedPhoenix: Int = 0

    @SerializedName("exp_phoenix")
    val expPhoenix: Int = 0

    @SerializedName("arrows_fired_phoenix")
    val arrowsFiredPhoenix: Int = 0

    @SerializedName("damage_phoenix")
    val damagePhoenix: Int = 0

    @SerializedName("time_played_phoenix")
    val timePlayedPhoenix: Int = 0

    @SerializedName("damage_taken_phoenix")
    val damageTakenPhoenix: Int = 0

    @SerializedName("chests_opened_phoenix")
    val chestsOpenedPhoenix: Int = 0

    @SerializedName("kills_phoenix")
    val killsPhoenix: Int = 0

    @SerializedName("potions_drunk_phoenix")
    val potionsDrunkPhoenix: Int = 0

    @SerializedName("arrows_hit_phoenix")
    val arrowsHitPhoenix: Int = 0

    @SerializedName("kills_teams_normal")
    val killsTeamsNormal: Int = 0

    @SerializedName("wins_teams_phoenix")
    val winsTeamsPhoenix: Int = 0

    @SerializedName("mobs_spawned_phoenix")
    val mobsSpawnedPhoenix: Int = 0

    @SerializedName("exp_necromancer")
    val expNecromancer: Int = 0

    @SerializedName("wins_solo_chaos")
    val winsSoloChaos: Int = 0

    @SerializedName("kills_solo_chaos")
    val killsSoloChaos: Int = 0

    @SerializedName("chests_opened_warrior")
    val chestsOpenedWarrior: Int = 0

    @SerializedName("time_played_warrior")
    val timePlayedWarrior: Int = 0

    @SerializedName("damage_taken_warrior")
    val damageTakenWarrior: Int = 0

    @SerializedName("damage_warrior")
    val damageWarrior: Int = 0

    @SerializedName("potions_thrown_warrior")
    val potionsThrownWarrior: Int = 0

    @SerializedName("exp_warrior")
    val expWarrior: Int = 0

    @SerializedName("games_played_warrior")
    val gamesPlayedWarrior: Int = 0

    @SerializedName("kills_warrior")
    val killsWarrior: Int = 0

    @SerializedName("exp_paladin")
    val expPaladin: Int = 0

    @SerializedName("exp_golem")
    val expGolem: Int = 0

    @SerializedName("exp_archer")
    val expArcher: Int = 0

    @SerializedName("potions_thrown_phoenix")
    val potionsThrownPhoenix: Int = 0

    @SerializedName("wins_phoenix")
    val winsPhoenix: Int = 0

    @SerializedName("exp_meatmaster")
    val expMeatmaster: Int = 0

    @SerializedName("taunt_kills_phoenix")
    val tauntKillsPhoenix: Int = 0

    @SerializedName("exp_random")
    val expRandom: Int = 0

    @SerializedName("PhoenixInventory")
    val phoenixinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("exp_baker")
    val expBaker: Int = 0

    @SerializedName("exp_creepertamer")
    val expCreepertamer: Int = 0

    @SerializedName("exp_pigman")
    val expPigman: Int = 0

    @SerializedName("exp_hunter")
    val expHunter: Int = 0

    @SerializedName("exp_fisherman")
    val expFisherman: Int = 0

    @SerializedName("arrows_fired_warrior")
    val arrowsFiredWarrior: Int = 0

    @SerializedName("potions_drunk_warrior")
    val potionsDrunkWarrior: Int = 0

    @SerializedName("arrows_hit_warrior")
    val arrowsHitWarrior: Int = 0

    @SerializedName("WarriorInventory")
    val warriorinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("wins_warrior")
    val winsWarrior: Int = 0

    @SerializedName("wins_teams_warrior")
    val winsTeamsWarrior: Int = 0

    @SerializedName("taunt_kills_warrior")
    val tauntKillsWarrior: Int = 0

    @SerializedName("mobs_spawned_warrior")
    val mobsSpawnedWarrior: Int = 0

    @SerializedName("exp_rogue")
    val expRogue: Int = 0

    @SerializedName("lastTourneyAd")
    val lasttourneyad: Int = 0

    @SerializedName("exp_scout")
    val expScout: Int = 0

    @SerializedName("mobs_spawned_scout")
    val mobsSpawnedScout: Int = 0

    @SerializedName("exp_farmer")
    val expFarmer: Int = 0

    @SerializedName("exp_troll")
    val expTroll: Int = 0

    @SerializedName("potions_drunk_donkeytamer")
    val potionsDrunkDonkeytamer: Int = 0

    @SerializedName("arrows_hit_donkeytamer")
    val arrowsHitDonkeytamer: Int = 0

    @SerializedName("arrows_fired_donkeytamer")
    val arrowsFiredDonkeytamer: Int = 0

    @SerializedName("wins_donkeytamer")
    val winsDonkeytamer: Int = 0

    @SerializedName("games_played_donkeytamer")
    val gamesPlayedDonkeytamer: Int = 0

    @SerializedName("potions_thrown_donkeytamer")
    val potionsThrownDonkeytamer: Int = 0

    @SerializedName("exp_donkeytamer")
    val expDonkeytamer: Int = 0

    @SerializedName("time_played_donkeytamer")
    val timePlayedDonkeytamer: Int = 0

    @SerializedName("chests_opened_donkeytamer")
    val chestsOpenedDonkeytamer: Int = 0

    @SerializedName("mobs_spawned_donkeytamer")
    val mobsSpawnedDonkeytamer: Int = 0

    @SerializedName("kills_donkeytamer")
    val killsDonkeytamer: Int = 0

    @SerializedName("damage_donkeytamer")
    val damageDonkeytamer: Int = 0

    @SerializedName("damage_taken_donkeytamer")
    val damageTakenDonkeytamer: Int = 0

    @SerializedName("wins_teams_donkeytamer")
    val winsTeamsDonkeytamer: Int = 0

    @SerializedName("taunt_kills_donkeytamer")
    val tauntKillsDonkeytamer: Int = 0

    @SerializedName("fall_kills_rogue")
    val fallKillsRogue: Int = 0

    @SerializedName("time_played_ranger")
    val timePlayedRanger: Int = 0

    @SerializedName("arrows_fired_ranger")
    val arrowsFiredRanger: Int = 0

    @SerializedName("damage_ranger")
    val damageRanger: Int = 0

    @SerializedName("chests_opened_ranger")
    val chestsOpenedRanger: Int = 0

    @SerializedName("games_played_ranger")
    val gamesPlayedRanger: Int = 0

    @SerializedName("kills_ranger")
    val killsRanger: Int = 0

    @SerializedName("exp_ranger")
    val expRanger: Int = 0

    @SerializedName("damage_taken_ranger")
    val damageTakenRanger: Int = 0

    @SerializedName("exp_horsetamer")
    val expHorsetamer: Int = 0

    @SerializedName("wins_farmer")
    val winsFarmer: Int = 0

    @SerializedName("taunt_kills_farmer")
    val tauntKillsFarmer: Int = 0

    @SerializedName("exp_warlock")
    val expWarlock: Int = 0

    @SerializedName("arrows_hit_warlock")
    val arrowsHitWarlock: Int = 0

    @SerializedName("wins_teams_ranger")
    val winsTeamsRanger: Int = 0

    @SerializedName("taunt_kills_ranger")
    val tauntKillsRanger: Int = 0

    @SerializedName("potions_drunk_ranger")
    val potionsDrunkRanger: Int = 0

    @SerializedName("arrows_hit_ranger")
    val arrowsHitRanger: Int = 0

    @SerializedName("mobs_spawned_ranger")
    val mobsSpawnedRanger: Int = 0

    @SerializedName("potions_thrown_ranger")
    val potionsThrownRanger: Int = 0

    @SerializedName("DonkeytamerInventory")
    val donkeytamerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("wins_ranger")
    val winsRanger: Int = 0

    @SerializedName("RangerInventory")
    val rangerinventory: Map<String, Any> = Collections.emptyMap()

    @SerializedName("exp_tim")
    val expTim: Int = 0

    @SerializedName("potions_thrown_farmer")
    val potionsThrownFarmer: Int = 0

    @SerializedName("exp_diver")
    val expDiver: Int = 0

    @SerializedName("potions_drunk_arachnologist")
    val potionsDrunkArachnologist: Int = 0

    @SerializedName("mobs_spawned_arachnologist")
    val mobsSpawnedArachnologist: Int = 0

    @SerializedName("kills_arachnologist")
    val killsArachnologist: Int = 0

    @SerializedName("chests_opened_arachnologist")
    val chestsOpenedArachnologist: Int = 0

    @SerializedName("time_played_arachnologist")
    val timePlayedArachnologist: Int = 0

    @SerializedName("damage_arachnologist")
    val damageArachnologist: Int = 0

    @SerializedName("damage_taken_arachnologist")
    val damageTakenArachnologist: Int = 0

    @SerializedName("games_played_arachnologist")
    val gamesPlayedArachnologist: Int = 0

    @SerializedName("exp_arachnologist")
    val expArachnologist: Int = 0

    @SerializedName("arrows_hit_arachnologist")
    val arrowsHitArachnologist: Int = 0

    @SerializedName("exp_hype train")
    val expHypeTrain: Int = 0

    @SerializedName("exp_slimeyslime")
    val expSlimeyslime: Int = 0

    @SerializedName("potions_drunk_toxicologist")
    val potionsDrunkToxicologist: Int = 0

    @SerializedName("kills_toxicologist")
    val killsToxicologist: Int = 0

    @SerializedName("mobs_spawned_toxicologist")
    val mobsSpawnedToxicologist: Int = 0

    @SerializedName("wins_toxicologist")
    val winsToxicologist: Int = 0

    @SerializedName("exp_toxicologist")
    val expToxicologist: Int = 0

    @SerializedName("arrows_hit_toxicologist")
    val arrowsHitToxicologist: Int = 0

    @SerializedName("arrows_fired_toxicologist")
    val arrowsFiredToxicologist: Int = 0

    @SerializedName("mobs_spawned_reddragon")
    val mobsSpawnedReddragon: Int = 0

    @SerializedName("exp_reddragon")
    val expReddragon: Int = 0

    @SerializedName("potions_thrown_reddragon")
    val potionsThrownReddragon: Int = 0

    @SerializedName("wins_teams_reddragon")
    val winsTeamsReddragon: Int = 0

    @SerializedName("wins_teams_warlock")
    val winsTeamsWarlock: Int = 0

    @SerializedName("exp_astronaut")
    val expAstronaut: Int = 0

    @SerializedName("exp_reaper")
    val expReaper: Int = 0

    @SerializedName("wins_teams_reaper")
    val winsTeamsReaper: Int = 0

    @SerializedName("wins_teams_toxicologist")
    val winsTeamsToxicologist: Int = 0

    @SerializedName("taunt_kills_toxicologist")
    val tauntKillsToxicologist: Int = 0

    @SerializedName("exp_shadow knight")
    val expShadowKnight: Int = 0

    @SerializedName("exp_guardian")
    val expGuardian: Int = 0

    @SerializedName("potions_thrown_jockey")
    val potionsThrownJockey: Int = 0

    @SerializedName("exp_jockey")
    val expJockey: Int = 0

    @SerializedName("wins_teams_jockey")
    val winsTeamsJockey: Int = 0

    @SerializedName("mobs_spawned_jockey")
    val mobsSpawnedJockey: Int = 0

    @SerializedName("exp_florist")
    val expFlorist: Int = 0

    @SerializedName("exp_armorer")
    val expArmorer: Int = 0

    @SerializedName("exp_wolftamer")
    val expWolftamer: Int = 0

    @SerializedName("arrows_hit_wolftamer")
    val arrowsHitWolftamer: Int = 0

    @SerializedName("exp_snowman")
    val expSnowman: Int = 0

    @SerializedName("wins_teams_snowman")
    val winsTeamsSnowman: Int = 0

    @SerializedName("wins_teams_wolftamer")
    val winsTeamsWolftamer: Int = 0

    @SerializedName("exp_blaze")
    val expBlaze: Int = 0

    @SerializedName("wins_snowman")
    val winsSnowman: Int = 0

    @SerializedName("taunt_kills_snowman")
    val tauntKillsSnowman: Int = 0

    @SerializedName("potions_thrown_snowman")
    val potionsThrownSnowman: Int = 0

    @SerializedName("mobs_spawned_reaper")
    val mobsSpawnedReaper: Int = 0

    @SerializedName("exp_viking")
    val expViking: Int = 0

    @SerializedName("exp_knight")
    val expKnight: Int = 0

    @SerializedName("taunt_kills_reddragon")
    val tauntKillsReddragon: Int = 0

    @SerializedName("wins_reddragon")
    val winsReddragon: Int = 0


}
